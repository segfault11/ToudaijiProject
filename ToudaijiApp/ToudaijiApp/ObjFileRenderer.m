...
@end
//------------------------------------------------------------------------------
@implementation ObjFileRenderer
//------------------------------------------------------------------------------
-(id)initWithFile:(ObjFilePtr)file andModel:(GLKMatrix4)model
{
    ...
    _renderData = (RenderData*)malloc(sizeof(RenderData)*numMat);
    _numRenderData = numMat;
    
    /* init the rendering data */
    for (int i = 0; i < numMat; i++)
    {
        initRenderData(&_renderData[i], file, i);
    }
...
-(void)dealloc
{
    for (int i = 0; i < _numRenderData; i++)
    {
        releaseRenderData(&_renderData[i]);
    }
...
-(void)render
{
    ...
    GLKMatrix4 proj = [cam getPerspective];
    GLKMatrix4 view = [cam getView];

    [program bind];
    [program setUniform:@"projection" WithMat4:proj.m];
    [program setUniform:@"view" WithMat4:view.m];
    [program setUniform:@"model" WithMat4:_model.m];
    
    for (int i = 0; i < _numRenderData; i++)
    {
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, _renderData[i].tex);
        glBindVertexArrayOES(_renderData[i].vertexArray);
        glDrawArrays(GL_TRIANGLES, 0, _renderData[i].count);
    }
